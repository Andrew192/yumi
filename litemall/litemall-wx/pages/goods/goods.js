...
Page({
  data: {
    ...
    userHasCollect: 0,
    number: 1,
    checkedSpecText: '사양 수량 선택',
    tmpSpecText: '사양 수량을 선택하세요',
    checkedSpecPrice: 0,
    openAttr: false,
    openShare: false,
    collect: false,
      ...
      success: function(res) {
        ...
        wx.saveImageToPhotosAlbum({
          ...
          success: function(res) {
            wx.showModal({
              title: '성공적으로 생성되었습니다.',
              content: '앨범에 성공적으로 저장되었으며 Moments에 공유할 수 있습니다，Moments에 공유할 수 있습니다.',
              showCancel: false,
              confirmText: 'OK',
              confirmColor: '#a78845',
              success: function(res) {
        ...
        if (_specificationList.length == 1) {
          if (_specificationList[0].valueList.length == 1) {
            ...
            if (_productPrice != _goodsPrice) {
              console.error('상품의 수량과 가격이 일치하지 않습니다.');
            }

            that.setData({
              checkedSpecText: _specificationList[0].valueList[0].value,
              tmpSpecText: '선택：' + _specificationList[0].valueList[0].value,
            });
          }
        }
  ...
  changeSpecInfo: function() {
    ...
    if (this.isCheckedAllSpec()) {
      ...
      if (!checkedProductArray || checkedProductArray.length <= 0) {
        ...
        console.error('사양에 해당하는 제품이 존재하지 않습니다.');
        return;
      }
  ...
  addFast: function() {
    ...
    if (this.data.openAttr == false) {
      ...
      //提示选择完整规格
      if (!this.isCheckedAllSpec()) {
        util.showErrorToast('전체 사양을 선택하십시오');
        return false;
      }
